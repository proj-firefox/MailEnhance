...
Section "-InstallStartCleanup"
  ...
  ; compatible with MailEnhance 2 (bug 533692).
...
Section "-Application" APP_IDX
  ...
  ; On install always add the MailEnhanceEML, MailEnhance.Url.mailto, and
  ; MailEnhance.Url.news keys.
  ...
  ; An empty string is used for the 5th param because MailEnhanceEML is not a
  ; protocol handler
  ${AddHandlerValues} "$0\MailEnhanceEML"  "$1" "$8,0" \
                      "${AppRegNameMail} Document" "" ""
  ${AddHandlerValues} "$0\MailEnhance.Url.mailto"  "$2" "$8,0" \
                      "${AppRegNameMail} URL" "true" ""
  ${AddHandlerValues} "$0\MailEnhance.Url.news" "$3" "$8,0" \
