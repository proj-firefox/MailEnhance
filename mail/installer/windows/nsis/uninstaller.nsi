...
Section "Uninstall"
  ...
  ${un.RegCleanAppHandler} "MailEnhance.Url.mailto"
  ${un.RegCleanAppHandler} "MailEnhance.Url.news"
  ${un.RegCleanAppHandler} "MailEnhanceEML"
  ...
  ClearErrors
  ReadRegStr $R9 HKCR "MailEnhanceEML" ""
  ; Don't clean up the file handlers if the MailEnhanceEML key still exists
  ; since there could be a second installation that may be the default file
  ; handler.
  ${If} ${Errors}
    ${un.RegCleanFileHandler}  ".eml"   "MailEnhanceEML"
    ${un.RegCleanFileHandler}  ".wdseml" "MailEnhanceEML"
  ...
  ; install of MailEnhance that is set as the
  ...
  ${un.CleanUpdatesDir} "MailEnhance"
  ...
  ; If MailEnhance.exe was successfully deleted yet we still need to restart to
  ; remove other files create a dummy MailEnhance.exe.moz-delete to prevent the
